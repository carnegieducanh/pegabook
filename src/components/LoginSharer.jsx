...
const LoginSharer = ({}) => {
    ...
    const handleLogin = async (event) => {
        ...
        fetch("https://pega-book-server.onrender.com/all-members")
            .then((res) => res.json())
            .then((data) => {
                const userSharer = data.find(
                    (user) =>
                        user.memberID === userName &&
                        user.sharerPassword === password
                );
                if (userSharer) {
                    alert("Bạn đã đăng nhập thành công");
                    navigate(`/sharer/dashboard/${userSharer._id}`);
                } else {
                ...
                <form onSubmit={handleLogin}>
                    <div className="space-y-6 ">
                        <h3 className="text-xl font-medium text-gray-900 dark:text-white">
                            Đăng nhập vào nền tảng của chúng tôi để{" "}
                            <span className="text-red-500">chia sẻ sách</span>{" "}
                            của bạn
                        </h3>
                        <div>
